...
#  and should be fixed via set loop counter before `for` to afterlast value.
#  May be fixed via `for outer i ...`
...
function parsereadwrite(str, pos)
    ...
    while true
        ...
        elseif cmdtaken && inparams && ttype == 'l'
            lex, pos = taketoken(str, pos)
            LEX = uppercase(lex)
            if LEX == "FMT" && peektoken(str, skipspaces(str, pos)) == '='
...
const FPreplacements = [
    ...
    #  something wrong with regxex #r"(?<!\w)((?:[+-]|)[0-9]+)_(\w+)"m => s"\1\3(\2)",
    #  something wrong with regxex #r"(?<!\w)((?:[+-]|)[0-9]+\.[0-9]+)_(\w+)"m => s"\2(\1)",
    #  something wrong with regxex #r"(?<!\w)((?:[+-]|)[0-9]+\.[0-9]+(?:[fFeE][+-]?[0-9]+))_(\w+)"m => s"\2(\1)",
    r"(\d)⊙(\d)"                                           => s"\1_\2",
